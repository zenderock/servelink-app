...
def process_env_vars(form, formdata):
    if hasattr(form, "env_vars"):
        form.env_vars.entries = [
            entry
            for entry in form.env_vars.entries
            if not entry.delete.data
            and (entry.key.data.strip() or entry.value.data.strip())
        ]
...
class ProjectForm(StarletteForm):
    name = StringField(
        _("Project name"),
        validators=[
            DataRequired(),
    ...
    def __init__(self, *args, db: AsyncSession, current_user=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.db = db
