...
from models import Project, Deployment, User, utc_now
...
@router.get("/repo-select", name="project_repo_select")
...
@router.get("/new-project", name="project_new")
async def new_project(request: Request):
    return TemplateResponse("projects/pages/new.html", {"request": request})


@router.api_route(
    "/new-project/details", methods=["GET", "POST"], name="project_new_details"
)
async def new_project_details(
    ...
    if not all([repo_id, repo_owner, repo_name, repo_default_branch]):
        ...
        return RedirectResponse(request.url_for("project_new"), status_code=303)

    form: Any = await ProjectForm.from_formdata(request, db=db, current_user=current_user)
    ...
    if request.method == "POST" and await form.validate_on_submit():
        ...
        return RedirectResponse(
            request.url_for("project_index", project_name=project.name), status_code=303
        )

    # Render template (GET or POST with errors)
    return TemplateResponse(
...
@router.api_route("/projects/{project_name}", name="project_index")
async def project_index(
    ...
    return TemplateResponse(
        ...
        context={
            ...
            "env_aliases": env_aliases,
            "secondary_navigation": "partials/_tabs-project.html",
            "latest_projects": latest_projects,
        },
    )


@router.api_route("/projects/{project_name}/deployments", name="project_deployments")
async def project_deployments(
    ...
    if request.headers.get("HX-Request") and fragment == "sse":
        return TemplateHTMXResponse(
            request=request,
            name="projects/partials/_deployments_sse.html",
...
@router.api_route(
    "/projects/{project_name}/deploy", methods=["GET", "POST"], name="project_deploy"
)
async def project_deploy(
    ...
    if not environment:
        ...
        return RedirectResponse(
            request.url_for("project_deploy", project_name=project.name),
            status_code=303,
        )
    ...
    return TemplateResponse(
        request=request,
        name="projects/partials/_deploy-dialog-commits.html",
        context={"project": project, "form": form, "commits": commits},
    )
...
async def project_settings(
    ...
    if request.method == "POST" and fragment == "danger":
        if await delete_project_form.validate_on_submit():
            try:
                ...
                return RedirectResponse(request.url_for("team_index"))
            except Exception as e:
                await db.rollback()
                logger.error(
        ...
        return RedirectResponse(
            url=str(request.url_for("project_settings", project_name=project.name))
            + "#danger",
            status_code=303,
        )
    ...
    general_form: Any = await ProjectGeneralForm.from_formdata(
        request, data={"name": project.name, "repo_id": project.repo_id}
    )

    if request.method == "POST" and fragment == "general":
        if await general_form.validate_on_submit():
            # Name
            ...
            if old_name != project.name:
                new_url = request.url_for("project_settings", project_name=project.name)
        ...
        if request.headers.get("HX-Request"):
            return TemplateHTMXResponse(
                request=request,
                name="projects/partials/settings/_general.html",
                context={"general_form": general_form, "project": project},
            )
    ...
    if (request.method == "GET" and not fragment) or fragment == "env_vars":
        ...
        if request.headers.get("HX-Request"):
            return TemplateHTMXResponse(
                ...
                context={"env_vars_form": env_vars_form, "project": project},
            )
    ...
    if request.headers.get("HX-Request") and fragment in (
        "environment",
        "delete_environment",
    ):
        return TemplateHTMXResponse(
...
@router.api_route(
    "/projects/{project_name}/deployments/{deployment_id}", name="project_deployment"
)
