...
from dependencies import get_redis_client, get_settings, get_github_client
...
async def deploy(ctx, deployment_id: str):
    ...
    async with AsyncSessionLocal() as db:
        try:
            deployment = (
                await db.execute(
                    select(Deployment).options(joinedload(Deployment.project))
            ...
            async with aiodocker.Docker(url=settings.docker_host) as docker_client:
                try:
                    ...
                    await redis_client.xadd(f"stream:project:{project.id}:deployment:{deployment.id}:status", fields)
                    ...
                    commands.append(f"echo 'Cloning {deployment.repo_full_name} (Branch: {deployment.branch}, Commit: {deployment.commit_sha[:7]})'")
                    installation = await get_installation_instance(project.github_installation_id, db, github_client)
                    commands.append(
                        "git init -q && "
                    ...
                    internal_network = await docker_client.networks.get('devpush_internal')
                    ...
                    while (time.time() - start_time) < timeout:
                        ...
                        container_ip = networks.get('devpush_default', {}).get('IPAddress')
                        
                        if not container_ip:
                            logger.info(f"Container {container.id} not yet assigned an IP address")
                    ...
                    branch = deployment.branch # Won't prevent collisions, but good enough
                    ...
                    project_config_file_path = os.path.join(settings.traefik_config_dir, f"project_{project.id}.yml")
                    write_config = False
                    traefik_config = None
                    ...
                    try:
                        if write_config and traefik_config:
                            ...
                            logger.info(f"Traefik dynamic config updated for project {project.id} at {project_config_file_path}")
                        else:
                            if os.path.exists(project_config_file_path):
                                os.remove(project_config_file_path)
                                logger.info(f"Removed Traefik config for project {project.id} from {project_config_file_path} (no active/valid aliases).")
                            else:
                                logger.info(f"No Traefik config to remove for project {project.id} (no active/valid aliases and file doesn't exist).")
                    except Exception as e_file_op:
                        logger.error(f"Error during Traefik config file operation for project {project.id}: {e_file_op}", exc_info=True)

                    # Success message
                    success_message = f"Deployment succeeded. Visit {deployment.url}"
                    await container.exec(['/bin/sh', '-c', f"echo '{success_message}' >> /proc/1/fd/1"])
                    ...
                    if container and deployment.conclusion == 'failed':
                        try:
                            ...
                            await db.commit()
                            logger.info(f"Cleaned up failed container {container.id} from deployment {deployment.id}")
                        except Exception as e:
                            logger.error(f"Error cleaning up container {container.id} from deployment {deployment.id}: {e}")

                    logger.info(f'Deployment {deployment.id} completed with conclusion: {deployment.conclusion}')
