...
from app.team.forms.project import ProjectForm, ProjectDeployForm, ProjectEnvironmentForm, ProjectEnvVarsForm, ProjectBuildAndProjectDeployForm, ProjectGeneralForm, ProjectDeleteEnvironmentForm, ProjectDeleteForm
from app.team.forms.team import TeamGeneralForm, TeamDeleteForm
from app.tasks.deploy import deploy
from app.helpers.github import get_installation_instance
from app.team.decorators import load_project, load_deployment
...
def new_project_details():
    ...
    if not repo_id or not repo_owner or not repo_name or not repo_default_branch:
        ...
        return redirect(url_for('team.new_project'))
...
def settings():
    ...
    # General
    general_form = TeamGeneralForm(data={
        'name': current_user.name,
        'slug': current_user.name.lower().replace(' ', '-')
    })
    ...
    if (request.method == 'GET' and not fragment) or fragment == 'general':
        if general_form.validate_on_submit():
            # Name
            old_name = team.name
            team.name = general_form.name.data
            ...
            if old_name != project.name:
                new_url = url_for('team.project_settings', project_name=project.name)
    ...
    # Delete team
    delete_team_form = TeamDeleteForm(data={'team_name': current_user.name})
...
def project_deploy(project):
    deployment_form = ProjectDeployForm(project)

    environment_choices = []
    for env in project.active_environments:
        environment_choices.append((env['id'], env['name']))
    deployment_form.environment_id.choices = environment_choices
    
    environment_id = request.args.get('environment_id', 'prod')
    environment = project.get_environment_by_id(environment_id)
    if not deployment_form.environment_id.data:
        deployment_form.environment_id.data = environment_id
    ...
    if deployment_form.validate_on_submit():
        try:
            ...
            if request.headers.get('HX-Request'):
                return '', 200, { 'HX-Redirect': url_for('team.deployment', project_name=project.name, deployment_id=deployment.id) }
            else:
                return redirect(url_for('team.deployment', project_name=project.name, deployment_id=deployment.id))
            
        except Exception as e:
            current_app.logger.error(f"Error deploying {project.name}: {str(e)}")
            flash(_('Failed to create deployment: %(error)s', error=str(e)), 'error')
    ...
    return render_template(
        'projects/partials/_deploy-dialog-form.html',
        project=project,
...
def project_settings(project, fragment=None):
    # Delete project
    delete_project_form = ProjectDeleteForm(data={'project_name': project.name})
    if request.method == 'POST' and 'delete_project' in request.form:
        ...
        for error in delete_project_form.confirm.errors:
            flash(error, 'error')
        return redirect(url_for('team.project_settings', project_name=project.name, _anchor='danger'))

    # General
    general_form = ProjectGeneralForm(data={
        'name': project.name,
        'repo_id': project.repo_id
    })
    ...
    env_vars_form = ProjectEnvVarsForm(data={
        'env_vars': [
            {
                'key': env.get('key', ''),
                'value': env.get('value', ''),
    ...
    # Environmentse
    environment_form = ProjectEnvironmentForm(project=project)
    delete_environment_form = ProjectDeleteEnvironmentForm(project=project)
    ...
    # Build and deploy
    build_and_deploy_form = ProjectBuildAndProjectDeployForm(
...
def inject_secondary_nav():
    def get_secondary_nav():
        ...
        if endpoint in ['team.index', 'team.projects', 'team.settings']:
            return 'partials/tabs/_account.html'
        elif endpoint in ['team.project', 'team.project_deployments', 'team.project_settings', 'team.deployment']:
